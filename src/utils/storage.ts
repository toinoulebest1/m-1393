...
export const getAudioFileUrl = async (filePath: string, deezerId?: string, songTitle?: string, songArtist?: string, songId?: string): Promise<{ url: string; duration?: number }> => {
  ...
  if (deezerId) {
    ...
    if (tidalId) {
      ...
      console.warn("⚠️ Impossible de trouver l'ID Tidal, passage à Deezmate");
    }
    ...
    try {
      ...
      if (response.ok || response.status === 405) { // 405 Method Not Allowed is ok, means endpoint exists but doesn't support HEAD
        console.log('✅ deezmate-proxy a répondu, utilisation de l\'URL.');
        return { url: proxyUrl }; // Pas de durée ici, on la découvrira à la lecture
      } else {
        const errorText = response.statusText;
        console.warn(`⚠️ deezmate-proxy a échoué avec le statut: ${response.status} ${errorText}`);
      }
    } catch (error) {
      console.warn('⚠️ Erreur lors de l\'appel à deezmate-proxy:', error);
    }
  }
        ...
        if (tidalId) {
          ...
          try {
            ...
            if (response.ok || response.status === 405) {
              console.log('✅ deezmate-proxy a répondu (recherche), utilisation de l\'URL.');
              // Mettre à jour le deezer_id dans la DB
              if (songId) {
              ...
              const errorText = response.statusText;
              console.warn(`⚠️ deezmate-proxy a échoué (recherche) avec le statut: ${response.status} ${errorText}`);
            }
          } catch (error) {
            console.warn('⚠️ Erreur lors de l\'appel à deezmate-proxy (recherche):', error);
          }
        }
      }
    } catch (error) {
