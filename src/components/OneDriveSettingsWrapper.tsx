...
import { getOneDriveConfig, isOneDriveEnabled, saveOneDriveConfig } from '@/utils/oneDriveStorage';
...
import type { OneDriveConfig } from '@/types/userSettings';
...
const OneDriveSettingsWrapper: React.FC = () => {
    ...
    if (migrationComplete) {
      const checkOneDriveStatus = async () => {
        try {
          ...
          console.log('OneDriveSettingsWrapper - Configuration OneDrive:', {
          ...
          setOneDriveStatus(finalStatus);
          setEnableOneDrive(hasValidToken);
  ...
  const handleQuickEnable = async () => {
    try {
      ...
      const adminOneDriveConfig = adminConfig?.value as any;
      if (!adminOneDriveConfig || 
          !adminOneDriveConfig.accessToken || 
          adminOneDriveConfig.accessToken === 'YOUR_DEFAULT_ACCESS_TOKEN') {
        console.error('Invalid admin configuration or token not configured');
        toast.error("La configuration admin n'est pas configurée. Un administrateur doit d'abord configurer les identifiants OneDrive.");
        setIsActivating(false);
        return;
      }
      ...
      const config: OneDriveConfig = {
        accessToken: adminOneDriveConfig.accessToken,
        ...
        expiresAt: adminOneDriveConfig.expiresAt,
        isEnabled: true
      };
      
      // Save the limited configuration to the user's settings
      await saveOneDriveConfig(config);
      toast.success("OneDrive a été activé avec succès avec le token admin");
      setOneDriveStatus('enabled');
      setEnableOneDrive(true);
      setHasToken(true);
      
      // Refresh page to apply changes
      setTimeout(() => window.location.reload(), 1500);
    } catch (error) {
      console.error("Error activating OneDrive:", error);
      toast.error("Une erreur est survenue lors de l'activation de OneDrive");
    } finally {
      setIsActivating(false);
    }
  };
  ...
  return (
    <>
        ...
        <div className="mb-4">
          <Alert className="bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800">
            ...
            <AlertDescription className="text-blue-800 dark:text-blue-200">
              <span className="font-semibold">Mode administrateur activé.</span> Vous pouvez configurer tous les paramètres OneDrive, y compris le refresh token.
          ...
          {oneDriveStatus === 'enabled' ? (
            <Alert className="bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800">
              ...
              <AlertDescription className="text-green-800 dark:text-green-200">
                ...
                Tous les nouveaux fichiers seront stockés sur OneDrive.
            ...
            <Alert className="bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800">
              ...
              <div className="flex flex-col space-y-2 w-full">
                <AlertDescription className="text-amber-800 dark:text-amber-200">
                  {isAdmin ? 
                    "OneDrive n'est pas configuré. Utilisez le formulaire ci-dessus pour configurer les paramètres admin." :
                    "OneDrive n'est pas activé. Vous pouvez utiliser le token configuré par l'administrateur en cliquant ci-dessous."}
                </AlertDescription>
            ...
            <Alert variant="destructive">
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription>
                Erreur lors de la vérification du statut OneDrive. Vérifiez la console pour plus d'informations.
              </AlertDescription>
            </Alert>
          )}
        </div>
      ...
      {/* Afficher le contenu de OneDriveSettings aux utilisateurs non-admin seulement si OneDrive est activé */}
      {(isAdmin || oneDriveStatus === 'enabled') && <OneDriveSettings />}
    </>
  );
};
