...
const DeezerSearchDialog = ({
  open,
  onClose,
  song,
  onUpdateSuccess
}: DeezerSearchDialogProps) => {
  const [query, setQuery] = useState<string>("");
  const [loading, setLoading] = useState<boolean>(false);
  const [results, setResults] = useState<any[]>([]);
  useEffect(() => {
    if (open && song) {
      const initialQuery = song.artist && song.artist !== "Unknown Artist" ? `${song.artist} ${song.title}` : song.title;
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke("deezer-search", {
        body: {
          query
        }
      });
  ...
  return <Dialog open={open} onOpenChange={isOpen => !isOpen && onClose()}>
      <DialogContent className="sm:max-w-xl lg:max-w-2xl bg-spotify-dark text-white border-white/20">
        <DialogHeader>
        ...
        <div className="space-y-4">
          <div className="flex space-x-2">
            <Input value={query} onChange={e => setQuery(e.target.value)} placeholder="Rechercher un titre, artiste..." className="flex-1 bg-spotify-dark/50 border-white/20 text-white" onKeyDown={e => e.key === 'Enter' && handleSearch()} autoFocus />
            <Button onClick={handleSearch} disabled={loading} className="bg-spotify-accent hover:bg-spotify-accent/80">
          ...
          {!loading && results.length > 0 && <div className="space-y-2 max-h-96 overflow-y-auto">
              <h4 className="text-sm font-medium text-gray-400">
                Résultats de recherche
              </h4>
              <div className="grid gap-2">
                {results.map(track => <div key={track.id} className="flex items-center gap-3 p-3 rounded-md hover:bg-white/10 border border-white/20 cursor-pointer transition-colors" onClick={() => handleApplyMetadata(track)}>
                    {track.album?.cover_small ? <img src={track.album.cover_small} alt={track.title} className="w-12 h-12 rounded object-cover" /> : <div className="w-12 h-12 rounded bg-white/10 flex items-center justify-center">
          ...
          {!loading && results.length === 0 && query !== "" && <div className="text-center py-8 text-gray-400">
